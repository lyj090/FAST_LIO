...
\begin{equation}
    \mathbf{G}=\left((\mathbf{a}\boxplus \mathbf{b})\boxplus \mathbf{c}\right)\boxminus \mathbf{d}\label{G_abcd}
\end{equation}
...
\begin{lemma}\label{lemma:partial}
    For \eqref{G_abcd}, devide it into blocks and there are two cases:

    {\bf Case 1}: If $\mathbf{a},\mathbf{b},\mathbf{c},\mathbf{d}\in\mathbb{R}^n$, \[\frac{\partial \mathbf{G}}{\partial \mathbf{b}}=\frac{\partial \mathbf{G}}{\partial \mathbf{c}}=\mathbf{I}_n.\] 

    {\bf Case 2}: If $\mathbf{G},\mathbf{b},\mathbf{c}\in\mathbb{R}^{3},\mathbf{a},\mathbf{d}\in SO(3)$, \[\frac{\partial \mathbf{G}}{\partial \mathbf{b}}=\mathbf{A}(\mathbf{G})^{-T}\mathrm{Exp}(-\mathbf{c})\mathbf{A}(\mathbf{b})^{T},\frac{\partial \mathbf{G}}{\partial \mathbf{c}}=\mathbf{A}(\mathbf{G})^{-T}\mathbf{A}(\mathbf{c})^{T}\]
    where $\mathbf{A}(\cdot)^{-1}$ is shown in \eqref{eq:attitude_matrix_inverse}.
\end{lemma}

\begin{proof}
    Case 1 is trivial, since the $\boxplus$ / $\boxminus$ operation on $\mathbb{R}^{n}$ are simply the general element-wise addition / subtraction.

    For Case 2, since $\mathbf{G}=((\mathbf{a}\boxplus \mathbf{b})\boxplus \mathbf{c})\boxminus \mathbf{d} =\mathrm{Log}(\mathbf{d}^{-1}\cdot((\mathbf{a}\boxplus \mathbf{b})\boxplus \mathbf{c}))$, we have 
...
With the above lemma (Noticing that $\mathbf{A}(\mathbf{0})=\mathbf{I}$; here we omit the index $i$ for simplicity)
\begin{align}
    \frac{\partial\mathbf{G}(\widetilde{\mathbf{x}},\mathbf{g}(\mathbf{0},\mathbf{0}))}{\partial\widetilde{\mathbf{x}}} |_{\mathbf{\widetilde{x}}_i=\mathbf{0}}
    &= \begin{bmatrix}
...
and
\begin{align}
    \frac{\partial \mathbf{G}(\mathbf{0},\mathbf{g}(\mathbf{0},\mathbf{w}))}{\partial \mathbf{g}(\mathbf{0},\mathbf{w})}|_{\mathbf{w}=\mathbf{0}} = \frac{\partial\mathbf{G}(\mathbf{0},\mathbf{g}(\widetilde{\mathbf{x}},\mathbf{0}))}{\partial\mathbf{g}(\widetilde{\mathbf{x}},\mathbf{0})}|_{\mathbf{\widetilde{x}}=\mathbf{0}} &=
...
Use the discrete dynamics in \eqref{eq:discrete_dynamics}, we have 
\begin{align}
