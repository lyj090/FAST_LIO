...
\begin{equation}
    \begin{aligned}
        ...
        \begin{bmatrix}
            ...
            \mathbf{0}&\mathbf{0}&\mathbf{0}&\mathbf{0}&\mathbf{0}&\mathbf{I}\end{bmatrix},\\
        \mathbf{F}_{\mathbf{w}}&=
        \begin{bmatrix}
    ...
    \end{aligned}
\end{equation}
The result is shown in \eqref{eq:linearized_matrices}, where $\widehat{\boldsymbol{\omega}}_i = \boldsymbol{\omega}_{m_i} - \widehat{\mathbf{b}}_{\boldsymbol{\omega}_i}$, $\widehat{\mathbf{a}}_i = \mathbf{a}_{m_i} - \widehat{\mathbf{b}}_{\mathbf{a}_i}$, and $\mathbf{A}(\mathbf{u})^{-1}$ follows the same definition in \cite{Bullo_Murray_1995} as below:
\begin{equation}
    \begin{split}
        \mathbf{A}(\mathbf{u})^{-1} = \mathbf{I} - \frac{1}{2} \lfloor\mathbf{u}\rfloor_\wedge + \left(1 - \alpha(\|\mathbf{u}\|)\right) \frac{\lfloor\mathbf{u}\rfloor_\wedge^2}{\|\mathbf{u}\|^2},\\
...
\[
    \check{\mathbf{x}}_{j-1} = \check{\mathbf{x}}_j \boxplus (-\Delta t \mathbf{f}(\check{\mathbf{x}}_j, \mathbf{u}_j, \mathbf{0})),
\]
...
\begin{equation}
    \begin{aligned}
        ...
        {}^{I_k}\check{\mathbf{v}}_{I_{j-1}} &= {}^{I_k}\check{\mathbf{v}}_{I_j} - {}^{I_k}\check{\mathbf{R}}_{I_j} (\mathbf{a}_{m_{i-1}} - \widehat{\mathbf{b}}_{{\mathbf{a}}_k}) \Delta t - {}^{I_k}\hat{\mathbf{g}}_k \Delta t, & \text{s.f. } {}^{I_k}\check{\mathbf{v}}_{I_m} &= {}^{G}\widehat{\mathbf{R}}_{I_k}^T {}^{G}\widehat{\mathbf{v}}_{I_k}, \quad {}^{I_k}\check{\mathbf{g}}_k = {}^{G}\widehat{\mathbf{R}}_{I_k}^T {}^{G}\widehat{\mathbf{g}}_k; \\
        {}^{I_k}\check{\mathbf{R}}_{I_{j-1}} &= {}^{I_k}\check{\mathbf{R}}_{I_j} \mathrm{Exp}((\widehat{\mathbf{b}}_{\boldsymbol{\omega}_k} - \boldsymbol{\omega}_{m_{i-1}}) \Delta t), & \text{s.f. } {}^{I_k}\check{\mathbf{R}}_{I_m} &= \mathbf{I}.
    \end{aligned}
    \label{eq:backward_propagation}
\end{equation}
where $\rho_{j-1} \in [\tau_{i-1}, \tau_i)$, $\Delta t = \rho_j - \rho_{j-1}$, and s.f. means “starting from”.

The backward propagation will produce a relative pose between time $\rho_j$ and the scan-end time $t_k$: ${}^{I_k}\check{\mathbf{T}}_{I_j} = ( {}^{I_k}\check{\mathbf{R}}_{I_j} , {}^{I_k}\check{\mathbf{p}}_{I_j} )$. This relative pose enables us to project the local measurement ${}^{L_j}\mathbf{p}_{f_j}$ to scan-end measurement ${}^{L_k}\mathbf{p}_{f_j}$ as follows 
(see Fig.~\ref{fig:forward_propagation}):
\begin{equation}
...
\begin{equation}
    \mathbf{0} = \mathbf{h}_j(\mathbf{x}_k, {}^{L_j}\mathbf{n}_{f_j}) = \mathbf{G}_j \left( {}^G\mathbf{T}_{I_k} {}^I\mathbf{T}_L {}^{L_k}\mathbf{T}_{L_j} ({}^{L_j}\mathbf{p}_{f_j} - {}^{L_j}\mathbf{n}_{f_j}) - {}^G\mathbf{q}_j \right).
...
\begin{align}
    \mathbf{0} &= \mathbf{h}_j \left( \mathbf{x}_k, {}^{L_j}\mathbf{n}_{f_j} \right) \nonumber \\
    &\simeq \mathbf{h}_j \left( \widehat{\mathbf{x}}_k^\kappa, \mathbf{0} \right) + \mathbf{H}_j^\kappa \widetilde{\mathbf{x}}_k^\kappa + \mathbf{v}_j \nonumber \\
...
The expression of $\mathbf{H}_j^\kappa$ (for planar feature points) is as below:
\begin{equation}
    \mathbf{H}_j^\kappa=\frac{\partial \mathbf{H}_j^\kappa}{\partial \mathbf{p}_j}\frac{\partial \mathbf{p}_j}{\partial \mathbf{\widetilde{x}}_j^\kappa}=[-{\mathbf{u}_j}{}^G\hat{\mathbf{R}}_I^\kappa\lfloor^I\mathbf{T}_L{}^L\mathbf{p}\rfloor_\wedge,\mathbf{u}_j,\boldsymbol{0},\boldsymbol{0},\boldsymbol{0},\boldsymbol{0}]
\end{equation}
...
\begin{equation}
    \mathbf{x}_k \boxminus \widehat{\mathbf{x}}_k = (\widehat{\mathbf{x}}_k^\kappa \boxplus \widetilde{\mathbf{x}}_k^\kappa) \boxminus \widehat{\mathbf{x}}_k = \widetilde{\mathbf{x}}_k^\kappa \boxplus (\widehat{\mathbf{x}}_k^\kappa \boxminus \widehat{\mathbf{x}}_k) = \widetilde{\mathbf{x}}_k^\kappa + \mathbf{J}^\kappa \widetilde{\mathbf{x}}_k^\kappa,
...
\begin{equation}
    ...
    \label{eq:jacobian_J}
\end{equation}
and $\mathbf{A}(\cdot)^{-1}$ is defined in \eqref{eq:attitude_matrix_inverse}. $\mathbf{J}^\kappa$ can be obtained using Lemma \ref{lemma:partial} in Appendix \ref{appendix:derivation}. For the first iteration (i.e., the case of extended Kalman filter), $\widehat{\mathbf{x}}_k^\kappa = \widehat{\mathbf{x}}_k$, then $\mathbf{J}^\kappa = \mathbf{I}$.
...
\end{equation}
where $\| \mathbf{x} \|_{\mathbf{M}}^2 = \mathbf{x}^T \mathbf{M} \mathbf{x}$. Substituting the linearization of the prior in \eqref{eq:prior_error} into \eqref{eq:map_estimation} and optimizing the resultant quadratic cost leads to the standard iterated Kalman filter \cite{lins}, which can be computed as follows (to simplify the notation, let $\mathbf{H} = [\mathbf{H}_1^{\kappa T}, \dots, \mathbf{H}_m^{\kappa T}]^T$, $\mathbf{R} = \mathrm{diag}(\mathbf{R}_1, \dots, \mathbf{R}_m)$, $\mathbf{P} = (\mathbf{J}^\kappa)^{-1} \widehat{\mathbf{P}}_k (\mathbf{J}^\kappa)^{-T}$, and $\mathbf{z}_k^\kappa = [ {\mathbf{z}_1^{\kappa T}}, \dots, {\mathbf{z}_m^{\kappa T}} ]^T$):
\begin{equation}
...
Since the LiDAR measurements are independent, the covariance matrix $\mathbf{R}$ is (block) diagonal and hence the new formula only requires to invert two matrices both in the dimension of state instead of measurements. The new formula reduces the computational complexity from $\mathcal{O}(m^3)$ to $\mathcal{O}(n^3)$, where $m$ is the number of measurements (feature points) and $n$ is the state dimension ($n=18$ in our case), thus greatly saves the computation as the state dimension is usually much lower than measurements in LIO (e.g., more than 1,000 effective feature points in a scan for 10 Hz scan rate while the state dimension is only 18). This efficiency is critical for real-time operation on resource-constrained platforms. This is proved in experimental section (Section \ref{sec:results}).

The state estimation is summarized in Algorithm \ref{alg:state_estimation}.
